...
#include "ESPWebSocketsServer.h"

ESPWebSocketsServerCore::ESPWebSocketsServerCore(const String & origin, const String & protocol) {
    _origin                 = origin;
    _protocol               = protocol;
    _runnning               = false;
    _pingInterval           = 0;
...
ESPWebSocketsServer::ESPWebSocketsServer(uint16_t port, const String & origin, const String & protocol)
    : ESPWebSocketsServerCore(origin, protocol) {
    _port = port;
    ...
    _server->onClient([](void * s, AsyncClient * c) {
        ((ESPWebSocketsServerCore *)s)->newClient(new AsyncTCPbuffer(c));
    },
        this);
#endif
}

ESPWebSocketsServerCore::~ESPWebSocketsServerCore() {
...
ESPWebSocketsServer::~ESPWebSocketsServer() {
}

/**
 * called to initialize the Websocket server
 */
void ESPWebSocketsServerCore::begin(void) {
...
void ESPWebSocketsServerCore::close(void) {
    _runnning = false;
    disconnect();
 ...
 */
void ESPWebSocketsServerCore::onEvent(WebSocketServerEvent cbEvent) {
    _cbEvent = cbEvent;
}
 ...
 */
void ESPWebSocketsServerCore::onValidateHttpHeader(
 ...
 * @return true if ok
 */
bool ESPWebSocketsServerCore::sendTXT(uint8_t num, uint8_t * payload, size_t length, bool headerToPayload) {
...
bool ESPWebSocketsServerCore::sendTXT(uint8_t num, const uint8_t * payload, size_t length) {
    return sendTXT(num, (uint8_t *)payload, length);
}

bool ESPWebSocketsServerCore::sendTXT(uint8_t num, char * payload, size_t length, bool headerToPayload) {
    return sendTXT(num, (uint8_t *)payload, length, headerToPayload);
}

bool ESPWebSocketsServerCore::sendTXT(uint8_t num, const char * payload, size_t length) {
    return sendTXT(num, (uint8_t *)payload, length);
}

bool ESPWebSocketsServerCore::sendTXT(uint8_t num, String & payload) {
 ...
 * @return true if ok
 */
bool ESPWebSocketsServerCore::broadcastTXT(uint8_t * payload, size_t length, bool headerToPayload) {
    WSclient_t * client;
    bool ret = true;
    if(length == 0) {
        length = strlen((const char *)payload);
...
bool ESPWebSocketsServerCore::broadcastTXT(const uint8_t * payload, size_t length) {
    return broadcastTXT((uint8_t *)payload, length);
}

bool ESPWebSocketsServerCore::broadcastTXT(char * payload, size_t length, bool headerToPayload) {
    return broadcastTXT((uint8_t *)payload, length, headerToPayload);
}

bool ESPWebSocketsServerCore::broadcastTXT(const char * payload, size_t length) {
    return broadcastTXT((uint8_t *)payload, length);
}

bool ESPWebSocketsServerCore::broadcastTXT(String & payload) {
 ...
 * @return true if ok
 */
bool ESPWebSocketsServerCore::sendBIN(uint8_t num, uint8_t * payload, size_t length, bool headerToPayload) {
    if(num >= WEBSOCKETS_SERVER_CLIENT_MAX) {
        return false;
    }
    WSclient_t * client = &_clients[num];
...
bool ESPWebSocketsServerCore::sendBIN(uint8_t num, const uint8_t * payload, size_t length) {
 ...
 * @return true if ok
 */
bool ESPWebSocketsServerCore::broadcastBIN(uint8_t * payload, size_t length, bool headerToPayload) {
    WSclient_t * client;
    bool ret = true;
    for(uint8_t i = 0; i < WEBSOCKETS_SERVER_CLIENT_MAX; i++) {
        client = &_clients[i];
...
bool ESPWebSocketsServerCore::broadcastBIN(const uint8_t * payload, size_t length) {
 ...
 */
bool ESPWebSocketsServerCore::sendPing(uint8_t num, uint8_t * payload, size_t length) {
    if(num >= WEBSOCKETS_SERVER_CLIENT_MAX) {
        return false;
    }
    WSclient_t * client = &_clients[num];
...
bool ESPWebSocketsServerCore::sendPing(uint8_t num, String & payload) {
 ...
 * @return true if ping is send out
 */
bool ESPWebSocketsServerCore::broadcastPing(uint8_t * payload, size_t length) {
    WSclient_t * client;
    bool ret = true;
    for(uint8_t i = 0; i < WEBSOCKETS_SERVER_CLIENT_MAX; i++) {
        client = &_clients[i];
...
bool ESPWebSocketsServerCore::broadcastPing(String & payload) {
 ...
 * disconnect all clients
 */
void ESPWebSocketsServerCore::disconnect(void) {
    WSclient_t * client;
    for(uint8_t i = 0; i < WEBSOCKETS_SERVER_CLIENT_MAX; i++) {
        client = &_clients[i];
        if(clientIsConnected(client)) {
            ESPWebSockets::clientDisconnect(client, 1000);
        }
    }
}
 ...
 * @param num uint8_t client id
 */
void ESPWebSocketsServerCore::disconnect(uint8_t num) {
    if(num >= WEBSOCKETS_SERVER_CLIENT_MAX) {
        return;
    }
    WSclient_t * client = &_clients[num];
 ...
 */
void ESPWebSocketsServerCore::setAuthorization(const char * user, const char * password) {
    if(user && password) {
        String auth = user;
        auth += ":";
        auth += password;
 ...
 */
void ESPWebSocketsServerCore::setAuthorization(const char * auth) {
    if(auth) {
 ...
 */
int ESPWebSocketsServerCore::connectedClients(bool ping) {
    WSclient_t * client;
    int count = 0;
    for(uint8_t i = 0; i < WEBSOCKETS_SERVER_CLIENT_MAX; i++) {
        client = &_clients[i];
 ...
 * @param num uint8_t client id
 */
bool ESPWebSocketsServerCore::clientIsConnected(uint8_t num) {
    if(num >= WEBSOCKETS_SERVER_CLIENT_MAX) {
        return false;
    }
    WSclient_t * client = &_clients[num];
 ...
 * @return IPAddress
 */
IPAddress ESPWebSocketsServerCore::remoteIP(uint8_t num) {
    if(num < WEBSOCKETS_SERVER_CLIENT_MAX) {
        WSclient_t * client = &_clients[num];
        if(clientIsConnected(client)) {
 ...
 * @param client
 */
WSclient_t * ESPWebSocketsServerCore::newClient(WEBSOCKETS_NETWORK_CLASS * TCPclient) {
    WSclient_t * client;
    ...
    for(uint8_t i = 0; i < WEBSOCKETS_SERVER_CLIENT_MAX; i++) {
        ...
        if(!clientIsConnected(client)) {
...
#if(WEBSOCKETS_NETWORK_TYPE == NETWORK_ESP8266_ASYNC)
            client->tcp->onDisconnect(std::bind([](ESPWebSocketsServerCore * server, AsyncTCPbuffer * obj, WSclient_t * client) -> bool {
            ...
            client->tcp->readStringUntil('\n', &(client->cHttpLine), std::bind(&ESPWebSocketsServerCore::handleHeader, this, client, &(client->cHttpLine)));
#endif
 ...
 * @param length size_t
 */
void ESPWebSocketsServerCore::messageReceived(WSclient_t * client, WSopcode_t opcode, uint8_t * payload, size_t length, bool fin) {
 ...
 */
void ESPWebSocketsServerCore::dropNativeClient(WSclient_t * client) {
    if(!client) {
        return;
    }
    if(client->tcp) {
 ...
 * @param client WSclient_t *  ptr to the client struct
 */
void ESPWebSocketsServerCore::clientDisconnect(WSclient_t * client) {
#if(WEBSOCKETS_NETWORK_TYPE == NETWORK_ESP8266) || (WEBSOCKETS_NETWORK_TYPE == NETWORK_ESP32)
 ...
 */
bool ESPWebSocketsServerCore::clientIsConnected(WSclient_t * client) {
    if(!client->tcp) {
        return false;
    }
 ...
 */
WSclient_t * ESPWebSocketsServerCore::handleNewClient(WEBSOCKETS_NETWORK_CLASS * tcpClient) {
 ...
 * Handle incoming Connection Request
 */
void ESPWebSocketsServer::handleNewClients(void) {
#if(WEBSOCKETS_NETWORK_TYPE == NETWORK_ESP8266) || (WEBSOCKETS_NETWORK_TYPE == NETWORK_ESP32)
 ...
 */
void ESPWebSocketsServerCore::handleClientData(void) {
    WSclient_t * client;
    for(uint8_t i = 0; i < WEBSOCKETS_SERVER_CLIENT_MAX; i++) {
        client = &_clients[i];
        if(clientIsConnected(client)) {
 ...
 */
bool ESPWebSocketsServerCore::hasMandatoryHeader(String headerName) {
    for(size_t i = 0; i < _mandatoryHttpHeaderCount; i++) {
 ...
 */
void ESPWebSocketsServerCore::handleHeader(WSclient_t * client, String * headerLine) {
    ...
    if(headerLine->length() > 0) {
        ...
        if(ok) {
            ...
            // send ping
            ESPWebSockets::sendFrame(client, WSop_ping);
 ...
 */
void ESPWebSocketsServerCore::handleHBPing(WSclient_t * client) {
 ...
 */
void ESPWebSocketsServerCore::enableHeartbeat(uint32_t pingInterval, uint32_t pongTimeout, uint8_t disconnectTimeoutCount) {
    ...
    for(uint8_t i = 0; i < WEBSOCKETS_SERVER_CLIENT_MAX; i++) {
        client = &_clients[i];
        ESPWebSockets::enableHeartbeat(client, pingInterval, pongTimeout, disconnectTimeoutCount);
    }
}
 ...
 */
void ESPWebSocketsServerCore::disableHeartbeat() {
    _pingInterval = 0;
 ...
 * called to initialize the Websocket server
 */
void ESPWebSocketsServer::begin(void) {
    ESPWebSocketsServerCore::begin();
    _server->begin();

    DEBUG_WEBSOCKETS("[WS-Server] Server Started.\n");
}

void ESPWebSocketsServer::close(void) {
    ESPWebSocketsServerCore::close();
 ...
 */
void ESPWebSocketsServerCore::loop(void) {
    if(_runnning) {
        WEBSOCKETS_YIELD();
        handleClientData();
    }
 ...
 * called in arduino loop
 */
void ESPWebSocketsServer::loop(void) {
    if(_runnning) {
        WEBSOCKETS_YIELD();
        handleNewClients();
        ESPWebSocketsServerCore::loop();
    }
}
#endif
