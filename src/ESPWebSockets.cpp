 ...
 */
void ESPWebSockets::clientDisconnect(WSclient_t * client, uint16_t code, char * reason, size_t reasonLen) {
 ...
 */
uint8_t ESPWebSockets::createHeader(uint8_t * headerPtr, WSopcode_t opcode, size_t length, bool mask, uint8_t maskKey[4], bool fin) {
    uint8_t headerSize;
 ...
 * @return true if ok
 */
bool ESPWebSockets::sendFrameHeader(WSclient_t * client, WSopcode_t opcode, size_t length, bool fin) {
 ...
 * @return true if ok
 */
bool ESPWebSockets::sendFrame(WSclient_t * client, WSopcode_t opcode, uint8_t * payload, size_t length, bool fin, bool headerToPayload) {
 ...
 * @param client WSclient_t *  ptr to the client struct
 */
void ESPWebSockets::headerDone(WSclient_t * client) {
 ...
 * @param client WSclient_t *  ptr to the client struct
 */
void ESPWebSockets::handleWebsocket(WSclient_t * client) {
 ...
 * @param size
 */
bool ESPWebSockets::handleWebsocketWaitFor(WSclient_t * client, size_t size) {
    if(!client->tcp || !client->tcp->connected()) {
        return false;
    }
    ...
    readCb(client, &client->cWsHeader[client->cWsRXsize], (size - client->cWsRXsize), std::bind([](ESPWebSockets * server, size_t size, WSclient_t * client, bool ok) {
...
void ESPWebSockets::handleWebsocketCb(WSclient_t * client) {
    if(!client->tcp || !client->tcp->connected()) {
        return;
    }
    ...
    if(header->payloadLen > 0) {
        ...
        readCb(client, payload, header->payloadLen, std::bind(&ESPWebSockets::handleWebsocketPayloadCb, this, std::placeholders::_1, std::placeholders::_2, payload));
    } else {
        handleWebsocketPayloadCb(client, true, NULL);
    }
}

void ESPWebSockets::handleWebsocketPayloadCb(WSclient_t * client, bool ok, uint8_t * payload) {
    WSMessageHeader_t * header = &client->cWsHeaderDecode;
    if(ok) {
        if(header->payloadLen > 0) {
 ...
 */
String ESPWebSockets::acceptKey(String & clientKey) {
 ...
 */
String ESPWebSockets::base64_encode(uint8_t * data, size_t length) {
 ...
 * @return true if ok
 */
bool ESPWebSockets::readCb(WSclient_t * client, uint8_t * out, size_t n, WSreadWaitCb cb) {
#if(WEBSOCKETS_NETWORK_TYPE == NETWORK_ESP8266_ASYNC)
    if(!client->tcp || !client->tcp->connected()) {
        return false;
    }
 ...
 * @return bytes send
 */
size_t ESPWebSockets::write(WSclient_t * client, uint8_t * out, size_t n) {
    if(out == NULL)
        return 0;
    if(client == NULL)
        return 0;
...
size_t ESPWebSockets::write(WSclient_t * client, const char * out) {
    if(client == NULL)
        return 0;
    if(out == NULL)
        return 0;
 ...
 */
void ESPWebSockets::enableHeartbeat(WSclient_t * client, uint32_t pingInterval, uint32_t pongTimeout, uint8_t disconnectTimeoutCount) {
    if(client == NULL)
        return;
 ...
 * @param client WSclient_t *
 */
void ESPWebSockets::handleHBTimeout(WSclient_t * client) {
